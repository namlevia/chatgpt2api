...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-stone-200/60 bg-white/80 backdrop-blur-xl transition-all duration-200",
        collapsed ? "w-16" : "w-56",
      )}
    >
      {/* Logo */}
      <div className="flex h-14 items-center gap-2 border-b border-stone-100 px-3">
        {!collapsed && (
          <Link href="/" className="text-[15px] font-bold tracking-tight text-stone-950">
            chatgpt2api
          </Link>
        )}
        <button
          ...
          className="ml-auto rounded-md p-1 text-stone-400 hover:bg-stone-100 hover:text-stone-600"
        >
      ...
      <div className="border-t border-stone-100 px-3 py-3">
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2 rounded-md text-xs text-stone-400 transition hover:text-stone-700",
