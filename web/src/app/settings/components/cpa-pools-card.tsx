        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-stone-400">先配置kết nối，再按需查询远程账号并chọn导入到本地号池。</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{pools.length} 个kết nối</Badge> : null}
            <Button className="h-9 rounded-xl bg-stone-800 px-4 text-white hover:bg-stone-800" onClick={openAddDialog}>
        ...
        {isLoadingPools ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-800 px-6 py-10 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-stone-400">点击「Thêm kết nối」Lưu你的 CLIProxyAPI 信息。</p>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {pools.map((pool) => {
              ...
              return (
                <div key={pool.id} className="flex flex-col gap-3 rounded-xl border border-stone-700 bg-stone-900 px-4 py-3">
