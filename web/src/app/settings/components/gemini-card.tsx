  ...
  return (
    <Card className="rounded-2xl border-stone-800 bg-stone-900/90 shadow-sm">
      <CardContent className="space-y-4 p-6">
        <div className="flex items-center gap-2"><Cpu className="size-5 text-stone-300" /><h3 className="text-sm font-semibold text-stone-100">Gemini AI Studio</h3></div>

        <label className="flex items-center gap-2 text-sm text-stone-300"><input type="checkbox" checked={geminiEnabled} onChange={(e) => setGeminiEnabled(e.target.checked)} className="size-4 accent-stone-400" /> Bật Gemini</label>
        ...
        <div className="space-y-2">
          ...
          <Textarea value={geminiKey} onChange={(e) => setGeminiKey(e.target.value)}
            ...
            className="min-h-24 rounded-xl border-stone-700 bg-stone-800 text-stone-100 font-mono text-xs placeholder:text-stone-400" />
          <p className="text-xs text-stone-400">Nhiều key → tự động round-robin khi hết quota. Lấy tại aistudio.google.com/apikey</p>
        </div>

        <div className="flex items-center gap-3">
          <label className="text-sm text-stone-300">Model:</label>
          <select value={geminiModel} onChange={(e) => setGeminiModel(e.target.value)}
            className="rounded-lg border border-stone-700 bg-stone-800 px-2 py-1.5 text-sm text-stone-100">
